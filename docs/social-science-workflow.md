...
The `social-science-workflow` domain is not a bag of tools; it is a **stage-gated pipeline** — an
orchestrator plus five discipline-enforcing gates that a study passes through *in order*, before it
is analyzed or written, and eleven analysis modules the orchestrator dispatches to. Each gate
refuses to advance until its one discipline is satisfied, and each writes to a shared **Design
...
| 2a | [`alterlab-ssci-measurement-gate`](../skills/social-science-workflow/alterlab-ssci-measurement-gate/) | (quantitative) A reliable measure of the wrong construct is still wrong — **reliability ≠ validity**. |
| 2b | [`alterlab-ssci-reflexivity-gate`](../skills/social-science-workflow/alterlab-ssci-reflexivity-gate/) | (qualitative) **Trustworthiness** — positionality + Lincoln & Guba, fail-closed. The measurement-stage analog for interpretivist work. |
...
The **measurement stage is paradigm-selected**: quantitative studies pass the measurement gate,
qualitative/interpretivist studies pass the reflexivity gate; mixed-methods runs both on their
strands.
...
The orchestrator dispatches to eleven pluggable analysis modules by `design_type` / data. Each is a
methods-discipline skill wrapping a **web-verified** library stack (versions pinned in each SKILL):

| Module | Method | Stack |
|--------|--------|-------|
| `alterlab-survey-analysis` | complex-sample design-based inference | samplics / svy; **R survey + srvyr** |
| `alterlab-causal-inference` | DiD / IV / RDD / panel FE / PSM / CATE | statsmodels, linearmodels, pyfixest, DoWhy, EconML, rdrobust |
| `alterlab-multilevel-models` | mixed-effects / hierarchical | statsmodels MixedLM, bambi; R lme4 / glmmTMB / brms |
...
| `alterlab-qualitative-analysis` | intercoder reliability, thematic/content coding | krippendorff, statsmodels; R irrCAC/icr (+ stdlib α) |
| `alterlab-meta-analysis` | effect-size pooling, heterogeneity, pub-bias | statsmodels meta_analysis; **R metafor** |
| `alterlab-missing-data` | MICE + Rubin's rules, FIML | statsmodels MICE; **R mice** |
...
The full spine is shipped: the **orchestrator**, the five **validity gates**, and the eleven
**analysis modules** — 17 skills. Every analysis-module library was web-verified against its
