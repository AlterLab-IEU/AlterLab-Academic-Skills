...
class CustomTrainer(Trainer):
    # Current signature includes num_items_in_batch (added so the Trainer can
    # normalize loss correctly under gradient accumulation). Accept it even if unused.
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        labels = inputs.pop("labels")
        outputs = model(**inputs)
        logits = outputs.logits

        # Custom loss computation (e.g. class-weighted cross-entropy)
