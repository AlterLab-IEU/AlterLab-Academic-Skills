...
# Notes/bonds/TIPS carry the auction rate in high_yield; bills use high_discnt_rate.
...
def get_auction_yields(term, n=24):
    ...
    df = pd.DataFrame(result["data"])
    df["yield"] = pd.to_numeric(df["high_yield"], errors="coerce")
    df["date"] = pd.to_datetime(df["record_date"])
...
print(df[["earning_period_start", "fixed_rate", "total_rate"]].head(10))
```
