...
2. **Run the 9 stages** — research → write → integrity check → review → revise →
   re-review → re-revise → final integrity check → finalize, dispatching
   `alterlab-deep-research`, `alterlab-paper-writer`, and `alterlab-paper-reviewer`
   as needed.
...
4. **Enforce integrity gates** — Citation existence and claim faithfulness must pass
   before review submission and again before finalize. Use `alterlab-citation-verifier`
   for that gate.
