...
- Use `alterlab-deep-research` (full mode) for comprehensive literature exploration
- Or use `alterlab-paper-writer` (plan mode) to start planning the paper directly
```
...
- The Research Plan Summary can be handed directly to `alterlab-paper-writer` (plan mode)
- If the user wants deeper literature exploration, suggest switching to `alterlab-deep-research` (full mode)
- `alterlab-paper-writer`'s `intake_agent` will automatically detect an existing Research Plan Summary and skip redundant steps
