...
compatibility: "Runs the scGPT model (`bowang-lab/scGPT`; PyPI `scgpt`, latest 0.2.4 released 2025-03, unchanged as of 2026-09) under `uv run python`. Its dependency pins are old (`scvi-tools<1.0`, `scanpy<2.0`, `torchtext`) and conflict with a current scverse stack, so install it in a dedicated environment rather than alongside alterlab-scanpy/alterlab-scvi-tools. Pretrained checkpoints are downloaded manually from the repo's Drive links (GB-scale); a CUDA GPU is strongly recommended. Input/output is AnnData (`.h5ad`). Dispatch heavy fine-tuning via alterlab-remote-compute."
metadata:
